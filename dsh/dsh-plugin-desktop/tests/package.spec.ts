...
describe('published package surface', () => {
  // `toContain` rather than upstream's exact match: this fork carries one more
  // workspace (openlux-plugin-account) and typechecks it first, so the root
  // command is a superset. The upstream tail must stay intact and in order.
  it('runs desktop and community market typechecks from the root command', () => {
    expect(workspaceManifest.scripts?.typecheck)
      .toContain('yarn workspace dsh-plugin-desktop typecheck && yarn workspace dsh-community-market typecheck')
  })
  ...
  // The four tests below guard this fork's own layer on the kernel bundles. They
  // exist because a resolutions block is easy to lose in an upstream merge: the
  // patch files survive, nothing references them, and the product silently ships
  // upstream's behaviour. Each one pins both resolution keys and one marker that
  // has to be observable in the installed bundle.
  it('makes model adoption a deliberate act on a 474-model route', () => {
    const patchPath = './patches/dsh-client-ui-settings-models@0.1.1-rc.2.patch'
    const installedClient = readFileSync(new URL(
      'node_modules/@deepseek-ai/dsh-client-ui-settings-models/lib/client.js',
      packageRoot,
    ...
    for (const marker of [
      ...
      'const pickable = shown.filter',
      'fetchPickedOnly',
      'fetchNoMatch',
      'zGbnIq_candidateAdded',
    ]) {
      expect(patch).toContain(marker)
      expect(installedClient).toContain(marker)
    }
  ...
  it('brands the conversation hero as a desktop build rather than a preview', () => {
    const patchPath = './patches/dsh-client-ui-conversation@0.1.1-rc.2.patch'
    expect(workspaceManifest.resolutions).toMatchObject({
      '@deepseek-ai/dsh-client-ui-conversation@npm:0.1.1-rc.2': expect.stringContaining(patchPath),
      '@deepseek-ai/dsh-client-ui-conversation@npm:^0.1.1-rc.2': expect.stringContaining(patchPath),
    })
    const installedClient = readFileSync(new URL(
    ...
    expect(installedClient).toContain('"hero.preview": "桌面版"')
    expect(installedClient).toContain('"hero.preview": "Desktop"')
    expect(installedClient).not.toContain('"hero.preview": "预览版"')
    expect(installedClient).not.toContain('"hero.preview": "Preview"')
  })
