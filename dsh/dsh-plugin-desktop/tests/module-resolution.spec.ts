...
describe('installProfilePackageResolver', () => {
  ...
  it('uses the same overlay for CommonJS package manifests resolved from the Profile anchor', () => {
    // Anchored like every other case in this file, and the two manifest paths
    // are derived rather than written out: the resolver compares
    // `parent.filename` against `fileURLToPath(profileBaseUrl)`, and a bare
    // POSIX anchor has no drive letter for that conversion to keep on Windows.
    const profileBaseUrl = 'file:///C:/Users/test/profile/package.json'
    const profileManifestPath = fileURLToPath(profileBaseUrl)
    const otherManifestPath = fileURLToPath('file:///C:/Users/test/another-profile/package.json')
