...
describe('ProfileCreateWindow', () => {
  ...
  it('creates one isolated window and focuses it on repeated opens', () => {
    ...
    expect(electron.windows[0]?.loadFile).toHaveBeenCalledWith(
      // The source hands Electron a filesystem path, so the separator is the
      // host's; a literal '/native-ui/…' only matches off Windows.
      expect.stringContaining(join('native-ui', 'profile-create.html')),
      { query: { locale: 'en' } },
    )
